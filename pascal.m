...
![words.bounds]byte wordlengths
[words.len]byte wordlengths
...
filehandle inp
ref char psource            !start of source file in memory
ref char pcc                !pointer to next char in source
...
proc main =
    ...
    if ncmdparams then
        infile:=addext(cmdparams[1],"pas")
    else
        println "Usage:"
        println "    pascal file[.pas]"
        stop
    fi
...
proc enter(ichar id, int k, t) =
    ...
    while not eqstring(itab[j].name,id) do
        j:=itab[j].link
    end
    ...
    pix:=&itab[ix]
    pix.name:=pcm_copyheapstring(id)
...
proc gen(instrrec i) =
    ...
    if cx+5>codemax then
        serror("Too many instructions")
    fi
    ...
    unless i.op in [kaddc, kadjs] and i.a=0 or i.op=kmulc and i.a=1 then
        p:=&code[cx-1]
    ...
    if sym in [period, lbrack] then
        ...
        while sym in [period, lbrack] do
            ...
            when period then
                ...
                j:=ttab[t].fields
                itab[0].name:=id
                while not eqstring(itab[j].name, id) do j:=itab[j].link od
...
proc statement =
    ...
    when forsym then
        ...
        i:=position()
        t:=itab[i].tip
!       if itab[i].kind<>varbl then error(130) fi
        error(130) when itab[i].kind<>varbl
        error(131) when ttab[t].kind<>simple
        getsym()
        skip(becomes)
        expression(x)
        mustbe(t,x)
...
RETURN
    for i:=0 to cx-1 do
        print i:"5",":   ", opcodenames[code[i].op]+1:"6 jl"
...
proc interpret =
    ...
    doswitchu ia:=code[pc].a; code[pc++].op
!   doswitch ia:=code[pc].a; code[pc++].op
    when kadd  then
        m[sp+1]:=m[sp+1]+m[sp]
        ++sp
...
proc init =
    for i:=0 to 255 do
        if i in 'a'..'z' or i in 'A'..'Z' or i in '0'..'9' then
            namemap[i]:=1
        fi
    od

    for i to words.len do
...
proc compile(ichar filename) =
    psource:=readfile(filename)
    if psource=nil then serror("Can't open file #", filename) fi
    pcc:=psource
