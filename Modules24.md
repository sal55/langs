...
### Emulating With a C Compiler

With my more recent C compiler, I tried to emulate one feature of this, which is the idea of listing all the C modules needed for a project at the start of the lead module.

(C doesn't have a module scheme and no concept of a 'lead module', other than perhaps the first module of several submitted ones. But with this experiment, there will only ever be one lead module; ergo, that becomes the lead module!)

The experiment I did listed the modules like this. So a project corresponding to my P A B C example might look like this `P.c` file:
````
#pragra module "A.c"
#pragra module "B.c"
#pragra module "C.c"
```
This can then be built like this:
````
mcc P
````
This is neat, but it would need a lot more work. Perhaps not using `#pragma`. Notice the module names are actually file-specs here.

Current alternates to building such a project include:
````
mcc P.c A.c B.c C.c   # List all the modules

mcc @P                # P is a text file containing the files

mcc -c P.c            # Compile independently then combine
mcc -c A.c
...

make                  # use a makefile (and not using mcc but a regular C compiler)

So building stuff in C is messy.
