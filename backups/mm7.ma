...
proc do_const(unit p) =
	...
	if ttisinteger[mode] or mode=tbool then
		...
		if p.isastring then
!CPL "doconst/CONST3", P.SVALUE, =p.slength, p.strtype
			if p.strtype='B' then gerror("1:B-str?") fi
!CPL "CONST/STR",P.SLENGTH
	...
	for pass:=1 to 2 do
		...
		do
			...
			when '\\' then			!escape char
				...
				when 't' then			!tab
					c:=9
				when 'u','v' then		!reserved for unicode, like \x but with 4/6 hex digits
...
!			if globalflag=export_scope and stmodule.subprogno<>1 then
		...
		when stdtypesym, krefsym, ptrsym, kicharsym, lsqsym, kslicesym then
dovar:
...
!		lex()
	...
	when krefsym, ptrsym then		!ref T
	retry:
...
!IF LHS.MODE>TLAST OR RHS.MODE>TLAST THEN
!CPL "EVAL BINOP > LAST", STRMODE(LHS.MODE), STRMODE(RHS.MODE), PCLNAMES[P.PCLOP]
!FI
...
module mnoos
module mwindllc
=== msyswini.m 0 0 56/99 ===
module msys
module mlib
module mclib
module mwindows
module mwindllc
=== msyswin_lib.m 0 0 57/99 ===
importdll msyswin =
    record fmtrec = 
        u8 minwidth
        i8 precision
    ...
    var i64 rd_length
    var ichar rd_pos
    var ichar rd_lastpos
    var i64 nsysparams
    var i64 ncmdparams
    var i64 nenvstrings
    var [128]ichar sysparams
    var ref [0:]ichar cmdparams
    var ref []ichar envstrings
    var i64 allocbytes
    var i64 fdebug
    var i64 rfsize
    var i64 smallmemtotal
    var [0..8]ref u64 freelist
    var [0..5]ichar pmnames
    var wt_handle hconsole
    var wt_handle hconsolein

    const i64 rd_buffersize = 16384
    const i64 maxblocksize = 2048
    const i64 pm_end = 0
    const i64 pm_option = 1
    const i64 pm_sourcefile = 2
    const i64 pm_libfile = 3
    const i64 pm_colon = 4
    const i64 pm_extra = 5
    const i64 c_eof = -1
    const i64 seek_set = 0
    const i64 seek_curr = 1
    ...
    proc strtofmt(ichar s,i64 slen,ref fmtrec fmt)
    func expandstr(ichar s,t,i64 n,ref fmtrec fmt) => i64
    func u64tostr(u64 aa,ichar s,u64 base,i64 sep) => i64
    func i64tostrfmt(i64 aa,ichar s,ref fmtrec fmt) => i64
    func u64tostrfmt(i64 aa,ichar s,ref fmtrec fmt) => i64
    func i64mintostr(ichar s,i64 base,sep) => i64
    func strtostrfmt(ichar s,t,i64 n,ref fmtrec fmt) => i64
    ...
    func m$sign_i64(i64 a) => i64
    func m$sign_r64(r64 x) => r64
    ...
    proc readrandom(filehandle handlex,ref u8 memx,i64 offset,size)
    func writerandom(filehandle handlex,ref u8 memx,i64 offset,size) => i64
    ...
    proc outu16(filehandle f,u64 x)
    proc outu32(filehandle f,u64 x)
    proc outu64(filehandle f,u64 x)
    ...
    func pcm_allocnfz(i64 n) => ref void
    proc os_init()
    ...
    func dirlist(ichar filespec,ref []ichar dest,i64 capacity,t=1) => i64
    func os_hpcounter() => i64
    func os_hpfreq() => i64
...
export byte pc_userunpcl=1
export byte ctarget=0
...
!	module mm_Assem.m
!	module mm_BlockC.m
	module mm_BlockPCL.m

	module mm_Decls.m
	module mm_Diags.m
!	module mm_Diags_dummy.m
!	module mm_Export_dummy.m
!	module mm_Exportq.m
!	module mm_Exportm.m
!	module mm_GenC.m
	module mm_GenPCL.m
...
!	module mm_LibC.m
	module mm_LibPCL.m
	module mm_Libsources.m
	module mm_Libsourcesc.m
!	module mm_Libsources_dummy.m
	module mm_Modules.m
	module mm_Name.m
	module mm_Parse.m
!	module mm_PCL.m
	...
	module mc_GenSS.m
	module MC_Decls.m
	module mc_OBJdecls.m
	module mc_Optim.m
	module mc_StackMCL.m
	module mc_WriteASM.m
!	module mc_WriteNASM.m
!	module mc_WriteASM_dummy.m
!	module mc_WriteC.m
	module mc_WriteEXE.m
	module mc_WriteOBJ.m
!	module mc_WriteSS.m
!	module mc_disasm.m
	file ---
...
!	file mm_parse.m
	...
	module mm_CLI.m
	module mm_Assem.m
	module mm_Assemaux.m
!	module mm_Assem_dummy.m
...
!	runexe pc -obj $ && ro $
!	runexe pc -EXE -showasm -showss $
!	runexe pc -pcl -showpcl -pst $
!	runexe pc -i  $
!	runexe pc -exe  $
!	runexe pc -exe -showasm -pst -no $
!	runexe pc -exe -showasm  $
	runexe pc -p -showpcl -pst $
!	runexe pc -asm -showasm -showpcl  $
!	runexe pc -asm -showasm   $
!	runexe pc -exe -showasm -showpcl  $
!	runexe aa $
...
11 mc_libc.m 0 0
12 mc_libmcl.m 0 0
13 mc_objdecls.m 0 0
14 mc_optim.m 0 0
15 mc_stackmcl.m 0 0
16 mc_writeasm.m 0 0
17 mc_writeexe.m 0 0
18 mc_writegas.m 0 0
19 mc_writenasm.m 0 0
...
37 mm_lib.m 0 0
38 mm_libpcl.m 0 0
39 mm_libsources.m 0 0
40 mm_libsourcesc.m 0 0
41 mm_modules.m 0 0
42 mm_name.m 0 0
43 mm_parse.m 0 0
44 mm_support.m 0 0
