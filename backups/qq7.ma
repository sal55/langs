...
!	compile_sp(nil, "println ""Hi There!!""")
	compile_sp("show.q")

	stopcode:=runprogram(subprogs[nsubprogs])

	stop stopcode
end
=== qq_generators.m 0 0 11/55 ===
!Generator Handling

global proc var_make_generator(variant a, dest, int n, symbol d) =
!d points to st entry for generator
!n is number of args, 0 or more, and a points to first of args (when 0, a=dest+1)
!dest is where to put the result. 
	object p
...
CPL "MAKEGEN",=D.NAME
	...
	p.genstacksize:=8
	p.genstack:=pcm_allocz(p.genstacksize*varsize)
	p.ngenparams:=n

	for i to n do
		(p.genstack+i-1)^:=a^			!xfer ownership
		++a
	od

	dest.tagx:=tgenerator ior hasrefmask
	dest.objptr:=p
end

!global proc var_call_gen(variant a, result) =
global proc var_call_gen(int skip) =
!skip is number of bytes the instruction for callgen takes;
!currently two for callhost/callgen
!Use that to store the next PCPTR after yield/return of G
	object p

	if sptr.tag<>tgenerator then
		pcerror("Not generator")
	fi

!	sptr.genmarker:=1			!indicate to Return this is was a generator call
	p:=sptr.objptr

	if p.genstate=0 then
		obj_startgen(p, skip)
	else
		obj_nextgen(p)
	fi
...
global proc obj_startgen(object p, int skip)=
!at this point, stack is (Void, GX)
!push nargs, then do whatever KCALL does to pass control
	symbol d:=p.stgen
	variant pargs:=p.genstack
	...
	++sptr
	sptr.tagx:=tretaddr
	SPTR.GENMARKER:=1			!part of tagx, otherwise zero
	sptr.retaddr := pcptr+skip
	...
	p.genstate:=1			!indicate started

!* Do CALLGEN(GX) -> STARTGEN(GD):
!  * Push Void
!  * Push GX
!  * CALLHOST => CALLGEN(GX)
!
!* STARTGEN
!  * Push stored params A,B:  (VOID, GX, A, B)
!  * Push TRETADDR with PCPTR+n (VOID, GX, VOID, A,B, RETADDR)
!  * Pass control entrypoint of proc: use GD.STGEN
!  * Any locals are allocated (say P,Q,R); stack is (VOID GX VOID, A, B, RETADDR, P,Q,R)
...
global proc obj_nextgen(object p)=
	CPL "NEXTGEN"
!	RESULT.TAGX:=TINT
!	RESULT.VALUE:=223344
end

global proc obj_free_gen(object p)=
	variant q

	q:=p.genstack

!	to p.genstacksize do
!		var_unshare(q)
!		++q
!	od

	if p.genstacksize then
		pcm_free(p.genstack, p.genstacksize*varsize)
	fi
...
=== qq_iterators.m 0 0 13/55 ===
!Generator Handling

!global proc var_make_iter(variant a, dest) =
!!d points to st entry for generator
!!n is number of args, 0 or more, and a points to first of args (when 0, a=dest+1)
!!dest is where to put the result. 
!!	object p, q
...
!	case a.tag
!	when tstring then
!		var_makeiter_string(a, dest)
!	when tlist then
!		var_makeiter_list(a, dest)
!	when tarray then
!		var_makeiter_array(a, dest)
!	when tdict then
!		var_makeiter_dict(a, dest)
!	when tset then
!		var_makeiter_set(a, dest)
!	else
!		pcerror("Bad Iterator target")
!	esac
!
!end
...
!dest is where to put the result. 
	...
	dest.tagx:=titerator ior hasrefmask
	dest.objptr:=p
end

global proc var_check_iter(variant a, dest) =
!skip is number of bytes the instruction for callgen takes;
!currently two for callhost/callgen
!Use that to store the next PCPTR after yield/return of G
	object p,q
	...
	if a.tag<>titerator then
		pcerror("Not iterator")
	fi

	p:=a.objptr

	case p.itertag
	when tstring, tlist, tarray then
!		var_checkiter_string(a, dest)

		dest.tagx:=tbool

		if p.iterpos>=p.iterupper then
			dest.value:=0
		else
			dest.value:=1
		fi
	when tdict then
		var_checkiter_dict(a, dest)
	when tset then
		var_checkiter_set(a, dest)
	else
		pcerror_s("checkiter not ready:",ttname[p.itertag])
	esac
end

global proc var_next_iter(variant a, dest) =
!skip is number of bytes the instruction for callgen takes;
!currently two for callhost/callgen
!Use that to store the next PCPTR after yield/return of G
	object p,q
	...
	case p.itertag
	when tstring then
		var_nextiter_string(a, dest)
	when tlist then
		var_nextiter_list(a, dest)
	when tarray then
		var_nextiter_array(a, dest)
	when tdict then
		var_nextiter_dict(a, dest)
	when tset then
		var_nextiter_set(a, dest)
	else
		pcustype("nextiter not ready",a)
	esac
end

global proc obj_free_iter(object p)=
	varrec v
	v.tagx:=p.itertag
	v.objptr:=p.objptr2
	var_unshareu(&v)
...
global proc var_dupl_iter(variant a)=
	object p, q
...
!
	...
	when kcallptr  then   ! Call X^; n is no. of params supplied; x is stack adjust
		...
		when tsymbol then
		when ttype then
			pclerror("Can't do dynamic casts")
		else
...
=== qq_syslibsdummy.m 0 0 35/55 ===
global const fsyslibs = 0

global func loadsysmodule(ifile pm)int=
	return 0
end
=== qq_tables.m 0 0 36/55 ===
!!---
...
importdll msvcrt=
	clang function 	opendir(cstring)ref byte
	clang function 	closedir(ref byte)ref byte
	clang function 	getcwd(ref byte,i32)ref byte
	clang function 	chdir(cstring)i32
	clang function 	mkdir(cstring,i32)i32
	clang function 	readdir(ref byte)ref byte
	clang function 	fnmatch(cstring,cstring,i32)i32
end
...
global func messagebox(a,mess,caption,d)=
RETURN PCERROR("LINUX MESSAGEBOX")
end

global func dirlist(filespec,?t)=
if t.isvoid then t:=1 fi			!files only
...
global func setcurrdir(newdir)=		!SETCURRDIR
return chdir(newdir)<>0
end

global func getcurrdir=		!GETCURRDIR
a:=new(array,byte,256)
p:=&a[1]
...
global func createdir(name)=
return system("mkdir "+name)=0
end

global func direxists(path)=
d:=opendir(path)
if d then
	closedir(d)
	return 1
...
9 qq_dicts.m 0 0
10 qq_embed.m 0 0
11 qq_generators.m 0 0
12 qq_host.m 0 0
13 qq_iterators.m 0 0
14 qq_lex.m 0 0
15 qq_lib.m 0 0
16 qq_lists.m 0 0
17 qq_modules.m 0 0
...
30 qq_show.m 0 0
31 qq_showpcl.m 0 0
32 qq_showpcldummy.m 0 0
33 qq_strings.m 0 0
34 qq_syslibs.m 0 0
35 qq_syslibsdummy.m 0 0
36 qq_tables.m 0 0
37 qq_vars.m 0 0
38 syswin.q 0 0
39 syslin.q 0 0
