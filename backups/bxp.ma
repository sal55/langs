...
global const langhomedir	= "C:/bxp/"
...
!		pgen((asfunc|kcallf|kcallp), gennameaddr_d(sysfnnames[fnindex]+3))
		pgen((asfunc|kcallf|kcallp), pgenextname(sysfnnames[fnindex]+3))
	end
	pccurr.nargs:=nargs
end
...
global proc genpc_callimp(ichar name, int isfn=0, unit a=nil, b=nil)=
	int nargs:=0, opc
	symbol d
	pcl p
	opc:=0
...
!
	...
	pgen((isfn|kcallf|kcallp), pgenextname(name))
...
proc do_stop(unit p, a) =
	genpc_callimp("exit", 0, (a|a|pzero))
end
...
!L2: 
	...
	pgen(kdupl)
	genpushint(0)
	pgencond(kjumpcc, ge_cc, pgenlabel(l3))
	pccurr.popone:=1
	genpushint(-1)
	...
	definefwdlabel(l3)
	genpushint(0)
	pgencond(kjumpcc, le_cc, pgenlabel(l4))
	genpushint(1)
	pgen(kresetmx)
	genjumpl(l2)
...
func addextname(ichar s)psymbol d=
...
global proc peephole(mcl m)=
	...
	if not fpeephole then return end
