...
importdll $cstd=
	...
	func setjmp		(ref void)i32
	proc longjmp	(ref void, i32)
...
!CPL "DOOPTION", OPTIONNAMES[SW], PASSLEVEL
	...
	when showrun_sw then fshowrun:=1
...
proc showcaption=
	println "M Compiler 8.4", $date, $time, "Opt:",foptimise
end
...
!
!	if fshowrun and passlevel=run_pass then
!		showrun(logdev)
!	end
...
!first symbol has been read
	...
	do
		...
		when stdtypesym, krefsym, kicharsym, lsqsym, ptrsym then
dovar:
...
!proc name(params) [=>]T [def]
	int kwd, startline, closesym, shortfun, isthreaded
	symbol stproc, stname

	kwd:=lx.symbol
	shortfun:=kwd=kfuncsym and lx.subcode=1
	isthreaded:=kwd=kprocsym and lx.subcode=1
	nforloops:=0
...
doproc:
		...
		when stdtypesym, namesym, krefsym, ptrsym, kicharsym, lsqsym then
...
!			opc:=kmutsym
		...
		when stdtypesym, krefsym, ptrsym, kicharsym, lsqsym then
...
!deal with most binary ops
	...
	when kidiv, kirem, kbitand, kbitor, kbitxor then
doidiv:
...
!t is the needed type for the element being indexed
	...
	if ttbasetype[amode]<>tarray then
		...
		when 'E' then			!end of this nested block
			...
			if f.mode=tbitfield then
				...
				if calign then
					...
					case alignment
					when 2 then
						if newoffset.odd then
alignerror:
							txerror_s("Bad calign:",f.name)
						end
					when 4 then
						if newoffset iand 3 then alignerror end
					when 8 then
						if newoffset iand 7 then alignerror end
					end
				end
...
proc tx_goto(unit p,a)=
	...
	if ttbasetype[m]<>tref or ttbasetype[tttarget[m]] not in [tlabel,tproc] then
...
!		if p.avcode then print @dev, " AV:", char(p.avcode), $ end
	...
	when jconst then
		...
		when tc64, tc8 then print @dev, mchr(a)
...
!
	...
	for i to nsourcefiles do
		...
		if fdeconsma then
			...
			writefile(name, sources[i].text, sources[i].size)
		end
	end
...
global proc txerror(ichar mess)=
!	error_gen('T',mess,p)
	error_gen('T',mess, nil)
end

global proc txerror_s(ichar mess,a)=
	[256]char str
	fprint @str,mess,a
!	error_gen('T',str,p)
	error_gen('T',str, nil)
...
!of separators so that successive alphanumeric items don't touch
	...
	when jindex,jdotindex,jdotslice then
		jevalx(a)
...
!	isfn:=p.tag=jcallfn
	...
	when jname then
		...
		IF D.ISTHREADED THEN GERROR("CALL TPROC") FI
...
!                       t  x          (attrs     )
	...
	(kretfn,       $+1, 1, 1,  0),  ! (t         ) Return from func with Z=retval
	...
	(kjumpret,     $+1, 1, 1,  0),  ! (L t       ) goto 0, common return point; deal with any ret value on stack
!	(kjumpretm,    $+1, 1, 0,  0),  ! (L t n     ) goto 0, common return point; deal with any ret value on stack
	(ksetcc,       $+1, 1, 1,  0),  ! (t c       ) Z' := Y cc Z
...
proc writepsymbol(psymbol d, ichar fmt)=
	...
	if d.segment then psstr(" "); psstr(segmentnames[d.segment]) fi
	if d.isthreaded then psstr(" Threaded ") fi

	psstr(" OFFSET:"); psstr(strint(D.OFFSET))
...
!allocate offsets to args, and set defines
	...
	if not info.isleaf or currfunc.isthreaded then
	...
	unless currfunc.isthreaded then
...
!	genmc(m_label, genlabel(retindex))
	...
	unless currproc.isthreaded then
...
!ax, bx refer to memory; do ax:=bx for n bytes
	...
	if n in [16, 32] then
		...
		if n=32 then
			ax:=applyoffset(ax, 16)
			bx:=applyoffset(bx, 16)
			genmc(m_movdqu, rx, bx)
			genmc(m_movdqu, ax, rx)
		end
...
proc scanprocinfo=
	...
	if currfunc.isthreaded then info.isleaf:=1 fi
...
doint:
				if currfunc.isthreaded then
					int oldflag:=regvarset.[nextireg]
					regvarset.[nextireg]:=0
					loadparam(j, mode, nextireg)
					regvarset.[nextireg]:=oldflag
				else
					loadparam(j, mode, nextireg)
				end
			end
...
global proc do_shift(pcl p, int opc)=
	...
	if checkregvarls(yy, p.next) and isimmint(zz) then		!optimise load-store of regvar
		ax:=getopnd(yy)
		currpcl:=p.next

		genmc(opc, ax, genint(pclopnds[zz].value))
...
global proc savepregs=
	int n, reg

	return when not currfunc.isthreaded
...
global enumdata [0:]ichar relocnames =
	...
	(addr32_rel,	"Abs_32"),
	(rel32_rel,		"Rel_32"),
	(rel8_rel,		"Rel_8"),				!used within assembler only, not in coff format
end
...
global proc axerror(ichar mess, mess2="")=
	fprintln "AX Error: # (#) Seq:#\n", mess, mess2, aaseqno
	stop 1
end
...
const labname="<Label>"				!for dummy label strecs 
...
proc genrex=
	...
	if nowmask or f2override or f3override then rex.[wbit]:=0 fi
...
global proc initlib(int nlabels)=
	...
	labeldeftable:=pcm_allocz(nlabels*^void.bytes)
end

global func getlabeldef(int labno)psymbol d=

	d:=labeldeftable[labno]

	if d=nil then

		d:=labeldeftable[labno]:=pcm_allocnfz(strec.bytes)
...
global ref byte pcode
global ref byte pidata
global ref byte pzdata
global ref byte pthunktable
global int pcodesize
global int pidatasize
global int pzdatasize
global int pthunksize

global record rimport =			!details about all imported symbols
	psymbol def					!full st entry
	ref proc fnaddr				!addr in external DLL
	int libindex
end

const maxrimports = 500
global [0..maxrimports]rimport rimporttable
global int nrimports

global [maxlibfile]i64 libinsttable
global [maxlibfile]ichar libinstnames
![maxlibfile]int libnotable			!index into dlltable

!the following is populated with the actual DLL addresses
!these called from thunk entries
global[maxrimports]ref proc importaddrtable

global psymbol stentrypoint

global proc runss(int cmdskip)=
	ref proc fnaddr
	...
	if fdorelocs then
		dorelocs(pcode, ss_coderelocs)
		dorelocs(pidata, ss_idatarelocs)
	end

!pass control to program
	if stentrypoint=nil then axerror("No entry point") end

	setcmdskip(cmdskip)


	fnaddr:=cast(pcode+stentrypoint.offset)
	fnaddr()
end

func findimportaddr(ichar name, int &libno)ref proc fnaddr=
	for i:=1 to nlibfiles when libinsttable[i] do
		fnaddr:=os_getdllprocaddr(libinsttable[i],name)
		libno:=i
		return fnaddr when fnaddr
	end

	axerror("Can't find external function:", name)
	nil
end
...
!the names: doing real linker work.)
	...
	while d, d:=d.next do
		if d.isimport and d.used then
			if nrimports>=maxrimports then axerror("genexe: Too many imports") end

			++nrimports
			fnaddr:=findimportaddr(d.name, libno)
			importaddrtable[nrimports]:=fnaddr

			rimporttable[nrimports].fnaddr:=fnaddr
			rimporttable[nrimports].def:=d
			rimporttable[nrimports].libindex:=libno
			d.importindex:=nrimports
...
proc initcode=
	...
	if highmem then axerror("run/himem") fi

	pcodesize:=bufferlength(ss_code)
	while pcodesize iand 7 do ss_code.pcurr++^:=0x90; ++pcodesize end
	pthunksize:=nrimports*8
!	pcodesize+:=pthunksize

	pcode:=os_allocexecmem(pcodesize+pthunksize)
	pthunktable:=pcode+pcodesize

	memcpy(pcode, ss_code.pstart, pcodesize)

	pidata:=ss_idata.pstart				!use idata as-is
	pidatasize:=bufferlength(ss_idata)
	pzdata:=pcm_allocz(ss_zdatalen)
	pzdatasize:=ss_zdatalen

!fill in thunk-table
	thunkptr:=pthunktable
	target:=cast(&importaddrtable)
!
	for i to nrimports do
		thunkptr++^:=0x48
		thunkptr++^:=0xFF
		thunkptr++^:=0x24
		thunkptr++^:=0x25
		(ref u32(thunkptr))^:=cast(target)
		thunkptr+:=4
		target+:=8
	end
end
...
!load library instances
	...
	for i to nlibfiles when libfiles[i]^<>'$' do
		...
		hinst:=os_getdllinst(filename)
		if hinst=0 then
			axerror("Can't load search lib", filename)
		end
		libinsttable[i]:=hinst
		libinstnames[i]:=pcm_copyheapstring(filename)
	end
...
proc dorelocs(ref byte p, ref relocrec r)=
	ref u32 p32
	ref u64 p64
	psymbol d
	int offset,index,thunkoffset,iatoffset
	...
	while r do
		d:=r.stdef
		index:=d.importindex				!into importtable
		thunkoffset:=pthunktable+(index-1)*8-pcode
...
!
		when addr32_rel, addr64_rel then				!for addr64, just leave top half zero
			if d.isimport then
				(ref u32(p+r.offset)^) :=u64(pcode+thunkoffset)
			else
				case d.segment
				when zdata_seg then u:=pzdata
				when idata_seg then u:=pidata
				when code_seg then u:=pcode
				else
					AXERROR("RELOCDATA/SEG?", D.NAME)

				end case
				p32:=cast(p+r.offset)
				if r.reloctype=addr32_rel then

!CPL "REL32", =P32^, =D.OFFSET
!					p32^ :=p32^+int(u+d.offset)
					p32^ :=p32^+int(u)
				else
					p64:=cast(p32)
!					p64^ :=p64^+int(u+d.offset)
					p64^ :=p64^+int(u)
				end
			end
		else
...
!	for i to nsymbols when symbolnametable[i]^='$' do
	...
	while d, d:=d.next do
		if d.isexport and
			(eqstring(d.name, "msys.$cmdskip") or
			eqstring(d.name, "$cmdskip")) then
			p:=cast(pzdata+d.offset)
			p^:=cmdskip
			return
		end
	end
...
27 mcl_stack.m 0 0
28 mcl_block.m 0 0
29 mcl_blockaux.m 0 0
30 mcx_decls.m 0 0
31 mcx_asm.m 0 0
32 mcx_lib.m 0 0
33 mcx_pedecls.m 0 0
34 mcx_optim.m 0 0
35 mcx_genss.m 0 0
36 mcx_exe.m 0 0
37 mcx_run.m 0 0
