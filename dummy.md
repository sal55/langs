...
I have two languages: Q and M as listed above. I prefer to write apps using the former, but programs run too slowly due to Q being dynamic and interpreted.
...
I have just completed **(1)**, and that's what this post is about. The next stage (the harder one) is still to come, and the results will be described in Part II if and when it is completed. (I didn't put 'Part I' in the title in case it doesn't happen; titles can't be edited!)
...
One invaluable feature if out of order declarations. If you see the example below, the 'Stack' array is at the end of the function, since the size isn't known until then.
...
````
I wanted to generate M code direct from AST2, but that wasn't practical, and not scalable to the more complicated things needed later. So I generated 'PCL' bytecode still, or a version of it, then converted a bytecode instruction at a time to M code.

**Type Annotations**
By themselves, these would only add concrete type info to AST terminals such as Name or Constant. To be useful, they need to propagate upwards. This requires an additional pass, so the pipeline becomes:
````
Source -> Parse -> AST1 -> Name resolve -> AST2 ->
...
* Rather than have a program-wide bytecode sequence, each function (and initialised data item) has its own PCL sequence
...
**Error Reporting** In the interpreter, it was easy to report error locations. That info does not exist in the M code. Instead, a global position variable it kept updated within the M code, but it is optional to keep the code size down.
...
**Speed** OK, this is the important bit. I've done experiments along these lines in the past, and already knew the code was not going to be magically fast just because it was 'native code'. Onthe  whole it is roughly the same speed as interpreted Q code. This set of results is from running the Fannkuch(10) benchmark. It is compared to some other products too:
````
          Seconds
...
`````
# Q code (I've added the type annotations but currently not used
