...
Q no longer generates monolithic bytecode files to package applications. But a new option `-qa` generates a single source file which can be run directly by the simple file interpreter. So just two files are need, and there are ways to combine them into one.

Python has third party tools to combine both the sprawling installation and all the relevant modules (not sure how that works as sometimes they are not known until runtime) into one executable, usually very larger, and usually with a slow start-up.

#### Common Syntax

Q share its syntax with its implementation language (my 'M' systems language). Compare C and Python.
...
```
In Python, all such variables, types anf functions are visible or can be visible from any module that imports this one.
...
Functions that don't return a result are called Procs and need a **sub** keyword rather than **fun**. This makes you think more carefully about what such a routine does, and can help catch errors.

#### Start and Main Functions

Unlike most languages of this kind, executable code goes inside functions. Q has two special function names:

**start** If any imported module has a start-function, it will be automatically called (call order of modules depends on info in the import header)

**main** If this is the lead module, then `main()` will be called as the program entry point.
...
Q allows **exit**, **next** and **redo**. **exit** corresponds to **break** in Python.

All of them work with nested loops too, unlike pretty much every other language.

#### Conditional Statements

Some simple control-flow statements - **exit** **next** **redo** **return** **goto** **stop** - can optionally be followed by a condition:
```
    exit if x=7
    return 0 when s=""
...
Such types are used when constructing arrays, structs, pointers to such types, and interfacing to FFI functions.

#### Arrays and Bit Arrays

Normal lists are variants, which are can be of any type. Arrays are lists of the same packed type. Bit arrays are sequences of 1, 2 or 4 bits. A billion-element bit array occupies 128MB. (I've no idea what support Python has for such arrays, if any, but I remember is has byte-arrays.)

#### Structs
A struct is a C-like record. It is defined like this:
```
    type cdate = struct
        byte day, month
        int16 year
...
This is necessary for working with FFIs. Python's version of this is done with addons and is very clunky with structs defined via function calls.
...
#### Maths Operators
A handful of operators are built-in, and they are actual operators, not functions. Which means among other things that no parentheses are needed. The built-in operators are:
```
    sqr
    sqrt
    sin, cos, tan
...
#### Unsigned integers
As mentioned, the default integer type (unless a number is too big) is int64. But an unsigned word64 type (ie. `u64`) also exists. This helps with some algorithms ported from C. And allows numbers between `2**63` and `2**64-1` without needing a Bignum.
...
```
    qq program           # normal execution
    qq -asm program      # use accelerator
```
Of course, performance is still at least a magnitude slower than statically typed and compiled code.
