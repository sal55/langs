...
This summaries the 2025/7.0 version of of my 'M' systems language.
...
- Uses a 'PCL' stack-based intermediate language, which now in its own backend library
- (The same backend library is shared with my C compiler)
- Simplified 2024 Module scheme
- Directly supports EXE DLL OBJ MX/ML ASM and NASM outputs
- Can run programs direcly from source code (as native code, or via interpreting the IL)
- Can generate high-loading and relocatable code for DLL/OBJ targets
- Has no real optimiser but is capable of storing some locals in registers
...
Using `-nosys` will leave out all of it, but then I/O will rely on C functions like `puts`,
which have to be declared using an explicit `module mlibc`

### Whole program compiler
The language is designed for whole-program compilation. A program is a single
EXE or DLL binary, or a single ASM, OBJ or MX file when that output is chosen.
...
````
    ...
    ASM     -a      .asm    Produce a single ASM file representing the whole program
    NASM    -nasm   .nasm   ASM file in NASM syntax (this needs to be configured
                            when building the compiler)
    MA      -ma     .ma     Produce a single-file, compilable source amalgamation
    Run     -r      --      Compile and run immediately; no file is written
    Run     -i      --      Compile and interpret the IL code (restrictions apply)

    MX      -mx     .mx     These are a private binary format; MX = EXE
    ML      -ml     .ml     ML = DLL, roughly
    PCL     -p      .pcl    Write discrete IL file

    C       -c      .pcl    Only availablle on older 'mc.exe' M->C transpiler project
````
...
````
    ...
    mm.exe      c:\mx           Main M compiler
    ms.exe      c:\mx           Copy of mm.exe; `-r -q` options are applied automatically

    pc.exe      c:\px           Standalone backend to process .pcl files
    pcl.dll     c:\px           Standalone library providing an API to generate IL

    aa.exe      c:\ax           x64 assembler, can handle .asm outputs of MM
...
self-contained and independent when compiling M projects to any of those outputs.

The `ms.exe` version (M-script) can used to run M programs just like a script.
The `-r -q` options otherwise needed are applied automatically.

Interpreting M code requires using `mmp.exe` and `pci.exe` instead.

Transpiling to C requires iuse of `mc.exe` instead (an old, partly deprecated project
that may be dropped).

`pc.exe` and `pcl.dll` are experimental and test products not much used; as the backend
is nearly always compled in the compilers.

### Targets
The primary target is x64 running Windows.

It wouldn't be that hard to target x64 under Linux, but there is little motivation
and little reason (plus SYS V ABI looks to be a nightmare).
...
project =
    ...
    linklib name            # 'ML' library (I think this has been dropped)
...
Other subprograms can have their own `main`; this is ignored. (This can be used when
...
````
    ...
    `exit(N)            Use C function (note exit is an M reserved word, backtick needed)
...
````
    ...
    export      Make visible to other subprogram. For the top
                program built into a DLL, export from the library
...
Unused parts of a word will be zeros. Escape codes (`\u \v`) can be used to specify Unicode 
characters, which are stored as multi-byte UTF8 sequences. Multiple Unicodes characters can be specified,
 subject to fitting within 64 bits.
...
For N-dimensional arrays, all dimensions need to be specified except for the first:
````
...
It also can only be used for fixed-size ojects. For dynamic objects, use M's `pcm_clear` function or C's `memset()` routines.
...
The Read-Only thing is poorly handled in my languages, which are keen on having mutable everything. But I am thinking of at leasting moving data like string constants to read-only segments (I don't have one in my back-end, so it may need to go into the code segment).
...
````
T is the return type. An optional `:` or `=>` can be just just before (see below).
...
(As noted above; this is not working.)
...
To use the library, M needs a set of bindings - function declarations written in M syntax, and inside an `importdll` block:
````
importdll msvcrt =
...
`-nosys` will compile without the library, and `-min` with a minimal test one. These options are mainly used for compiler development.
...
### Optimising
There is no real optimiser. There are a couple of weak attempts:

* 5/6 GP registers in every function are set aside for locals and args. First-come first served.
* There are perhaps 8 FP registers for the same purposes
* There is small peephole optimiser, but the main function there is for smaller code

There are no overheads in applying these (whatever overheads are offset by having less code to
process later), so they are enabled by default.

There is a problem with inline assembler: the register allocation is disable when ASM is used,
as it won't know where variables are kept or which registers are used.

(I have a plan to deal with that. It needs the means to refer to variables in assembly that
will work wherever the variable is stored. And assembly needs to avoid writing to non-volatile 
except when accessing variables.)
...
There will be some program the developer uses to browse, edit, build and do test runs.
...
This is not great. When a poor error report, or even lack of detection, has caused grief, then I
generally get it fixed.
...
There isn't a formal or even informal grammar. The top-level syntax of an M program is
roughly:
````
