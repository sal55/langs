...
The Q compiler/interpreter is a single executablem currently 0.8MB which includes the standard library (admittedly not too big, however it will still be a single file).
...
**goto** is in all my languages, because it can be very handy, even it is rarely used (eg. to port an algorithm using goto or a language that used goto)
...
```
    ...
    host.fnb(30,40)
```
No other declarations are needed. Inside the host, callable functions like this are simply marked as 'export' rather than 'global'; then interface data is automatically generated by the compiler of the interpreter (a feature of the language used).
