mafile 31
  1 mm.m                 382     1363   0
  2 mm_decls.m         13197     1770   0
  3 clibnew.m           3397    14991   0
  4 mm_tables.m        44039    18414   0
  5 mm_mcldecls.m      13336    62481   0
  6 mm_start.m         19652    75842   0
  7 msysnew.m          47147    95518   0
  8 mlib.m             26724   142686   0
  9 oswindows.m        12536   169436   0
 10 mm_support.m       13445   182000   0
 11 mm_lib.m           38755   195469   0
 12 mm_lex.m           36699   234248   0
 13 mm_diags.m         13226   270973   0
 14 mm_genwx64.m        4343   284227   0
 15 mm_genpcl.m         9569   288597   0
 16 mm_libpcl.m        24908   298193   0
 17 mm_blockpcl.m      69941   323130   0
 18 mm_genmcl.m        91934   393098   0
 19 mm_libmcl.m        41829   485059   0
 20 var_tables.m        3540   526916   0
 21 ma_genss.m         46359   530482   0
 22 ma_decls.m          1674   576867   0
 23 ma_lib.m            2262   578565   0
 24 ma_objdecls.m       2566   580856   0
 25 ma_writeobj.m       7676   583451   0
 26 ma_writeexe.m      26430   591156   0
 27 ma_disasm.m        25847   617613   0
 28 mm_parse.m         88793   643486   0
 29 mm_name.m          18181   732304   0
 30 mm_type.m          64868   750510   0
 31 mm_help.txt          866   815405   1
=== mm.m 1/31 ===
...
global record opndrec =		!up to 32 bytes
	...
	byte SPARE			!extra offset to label for mem/imm modes
...
global proc addmodulemapping(ichar old, newx, optionname=nil, valuename=nil)=
	int option
!
!CPL "ADDMAPPING",OLD,NEWX,OPTIONNAME,VALUENAME
...
proc addsearchlib(ichar name) =
	...
	if nsearchlibs>=searchlibs.len then
		loaderror("Too many LIB files")
	fi
!CPL "ADDSEARCH",name
...
!CPL "DONE"
	...
	case p.tag
	when j_eq,j_add then
		gerror("use EVAL to evaluate this expression (or ""="" used in place of "":=""?)")
	esac
...
!CPL PCLNAMES[P.OPCODE],P.LINENO IAND 16777215, SOURCEFILENAMES[P.LINENO>>24]
	...
	bb:=&p^.b
!IF P.CATMODE=0 THEN
!	CPL "CATMODE=0",PCLNAMES[P.OPCODE]
!ELSE
!	CPL "	NOT 0"
!FI
...
proc pc_storemem_x4(ref pclrec p) =
	getopnds(1)
	genmc_storemem_x4(p^.a.def)
end
...
proc pc_absto_i64_u64(ref pclrec p) =
	ref opndrec px,lx
	int lab
	getopnds(1)
	genmc(m_cmp,px:=genopndind(xa),genint_mc(0))
	lab:=++labelno
...
proc pc_absto_r32_r64(ref pclrec p) =
	ref opndrec px,fx

	getopnds(1)
	px:=genopndind(xa)

	px^.offset+:=ttsize[p.mode]-1		!+3 or +7
	genmc(m_andx,px,genint_mc(0x7F))
...
IF REGIX AND REG=0 AND A.SCALE<=1 THEN
	REG:=REGIX
	REGIX:=0
FI
...
!finish pointing at ")"
	...
	if lx.symbol in [koutsym,addrsym] then
		...
		pmprefix:=1
		lex()
	elsif lx.symbol=opsym and lx.subcode=j_in then
		parammode:=in_param
		pmprefix:=1
		lex()
	fi
...
!CPL "RPTYPES",OWNER.NAME
		...
		storemode(8,owner,m,&stname^.mode)
		stname^.parammode:=parammode
		addlistparam(&stlist,&stlistx,stname)
		parammode:=var_param
...
when constid,staticid,frameid,paramid then
	...
	if p^.code AND P.NAMEID<>FRAMEID then
!CPL "DEFLIST SCANNING CODE FOR",P.NAME
...
!CPL "	RESOLVETOPEMAME",STNEWNAME.NAME,owner.name
...
!CPL POWNER.NAME,NAMENAMES[POWNER.NAMEID]
...
when j_negto, j_absto, j_inotto, j_notlto then
	if b then txerror("Extra operand for unary op") fi
	tx_negto(p,a)
...
if d^.islet and lv then
	txerror_s("Let: can't use # as lvalue",d.name,p)
fi
