// Load and run .mx files
// Compile usin Tiny C on Windows:
//   tcc runmx.c
...
u64 callcrt(u64 name, u64* args, int nargs) {
    puts("CALLCRT NOT READY");
    exit(1);
    return 0;
}

int needcrt(librec* lib) {
    if (!crtmode) return 0;
    if (strcmp(lib->libname,"crt")==0) return 0;
    return 1;
}
...
void setspecialglobals(int cmdskip) {
    for (int i=1; i<=nsymbols; ++i) if (*symbolnametable[i]=='$') {
        ...
        } else if (strcmp(symbolnametable[i], "$callcrt")==0) {
            *(void**)symboladdress[i] = (void*)callcrt;
        }
    }
}
