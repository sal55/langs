...
GLenum doubleBuffer=0;
GLint windW = 900, windH = 600;
...
NewStar(GLint n, GLint d)
{
  ...
  stars[n].x[0] = (float) (myrand() % MAXPOS - MAXPOS / 2);
  stars[n].y[0] = (float) (myrand() % MAXPOS - MAXPOS / 2);
  stars[n].z[0] = (float) ((myrand()) % MAXPOS + d);
  stars[n].x[1] = stars[n].x[0];
  stars[n].y[1] = stars[n].y[0];
  stars[n].z[1] = stars[n].z[0];
  if (myrand() % 4 == 0 && flag == WEIRD) {
    stars[n].offsetX = (float) (myrand() % 100 - 100 / 2);
    stars[n].offsetY = (float) (myrand() % 100 - 100 / 2);
    stars[n].offsetR = (float) (myrand() % 25 - 25 / 2);
  } else {
    stars[n].offsetX = 0.0;
    stars[n].offsetY = 0.0;
    stars[n].offsetR = 0.0;
