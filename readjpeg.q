...
proc start=
CPL "JPEG START NEW st handling"

!file:="\\jpeg\\card2.jpg"				!2x2
file:="card2.jpg"				!2x2
!file:="/jpeg/girl.jpg"				!2x2
!file:="\\jpeg\\g.jpg"				!2x2
!file:="\\jpeg\\smallgirl.jpg"				!2x2
!file:="/jpeg/horse.jpg"				!1x1
!file:="/jpeg/lego.jpg"				!2x1 640x480
!file:="/jpeg/fred.jpg"				!2x1 640x480
!file:="/jpeg/mona.jpg"
!file:="\\jpeg\\freya.jpg"
!file:="\\jpeg\\wtc-photo.jpg"
!file:="/jpeg/small.jpg"
!file:="/jpeg/small2.jpg"
!file:="/jpeg/broken.jpg"
!file:="\\jpeg\\6502.jpg"
...
CPL "IMAGE:",HDR.WIDTH,"x",HDR.HEIGHT
CPL
...
!	w:=gxcreatewindow(dim:(dimx*2,dimy*3%2))
!	gxcopy(w,bm,scalex:scale*4, SX:200, SY:250)
...
f:=createfile(changeext(file,"ppm"))

CPL "Write PPM:",CHANGEEXT(FILE,"ppm")
...
!println @f,"#comment"
!println @f,"#comment"
!println @f,height, 255
!println @f,"255 #comment"
println @f,"255 #comment"
!println @f
!
buffer:=data

buffer:=malloc(hdr.framebytes)
if buffer=0 then return 0 fi
buffer:=makeref(buffer,byte)
memcpy(buffer,data,hdr.framebytes)
...
CPL
CPL "***** JPEG HEADER: *****"
cpl =hdr.width,=hdr.height
cpl =hdr.ncomponents
cpl =hdr.framebytes
cpl =hdr.qtable
cpl =hdr.vsample
cpl =hdr.hsample
cpl =hdr.usedc,=hdr.useac,=hdr.useq
cpl =hdr.dctable
cpl =hdr.actable
end
...
do
	...
	if c=0xFF then		!marker
		...
		when 0xC0 then
			CPL pref,"FFC0 SOF0 baseline"
			read_sof(fs)
		when 0xC2 then
			CPL pref,"FFC2 SOF2 progressive"
		when 0xC4 then
			CPL pref,"FFC4 DHT"
			read_dht(fs)
CPL "DONE"
		when 0xDB then
			...
			read_dqt(fs)
		when 0xDD then
			CPL pref,"FFDD DRI RST interval"
			readword(fs)				!skip length
			hdr.dri:=readword(fs)
		when 0xEE then
			CPL pref,"FFEE COM comment"
		when 0xDA then
			CPL pref,"FFDA SOS"
			read_sos(fs)
		...
		when 0xD9 then
			CPL pref,"FFD9 EOI"
		when 0x0 then
			CPL pref,"FF00 <embedded FF data>"
		when 0xFF then
			CPL pref,"FFFF padding"
		else
			CPL pref,"FF",,c:"2zh","Unknown marker"
		esac
	else
	fi
od
...
when 3 then
	...
	if hsample2=vsample2=hsample3=vsample3 and (hsample1<=2 and vsample1<=2) then
		...
		cpl hsample1,vsample1,hsample2,vsample2,hsample3,vsample3
