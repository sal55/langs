...
proc main=
	...
	if nsysparams>=2 then
		...
		println "Usage:",sysparams[1],"<jpeg file>"
		stop
	fi

	data:=loadjpeg(file,&width,&height)
	...
	writeppm(file, data, width, height)

end

proc writeppm(ichar file,ref byte data,int width,height)=
	filehandle f
	ref byte p, q
	int linebytes

	f:=fopen(file:=changeext(file,"ppm"),"wb")
	...
	writerandom(f,data,getfilepos(f),width*height*3)
	fclose(f)
end
...
!global callback function "DllMain"(int hinst,int reason, reserved)int=
...
proc reconsblockcolour(ref[]int lum1,lum2,lum3,lum4,cr,cb, ref byte data,int x,y, hoz,vert)=
	...
	for i:=0 to ilim do
		if (yy:=y+i)>=height then nextloop fi
		p:=data+(yy*width+x)*3

		for j:=0 to jlim do
			if x+j>=width then nextloop fi
			if i<8 then
				if j<8 then
