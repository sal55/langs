...
proc start=
	...
	if showbm then
		...
		while dimx>1920 or dimy>1080 do
			dimx := dimx%2
			dimy := dimy%2
			scale /:=2.0
		od
...
func buildhufftree(codelength,symbols)=
	tot:=0
	for x in codelength do
		tot+:=x
	od
...
func buildcanonical(codelengths)=
	n:=0
	for x in codelengths do
		n+:=x
	od
...
func loadcolour(fs,hdr,hoz,vert)=
