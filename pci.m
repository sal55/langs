...
macro getvalue          = pcltab[pc].value
macro getsvalue         = pcltab[pc].svalue
macro getvalue3         = pcltab[pc+2].value
macro getxvalue         = pcltab[pc].xvalue
macro getxvalue32       = pcltab[pc].xvalue32
macro getlabelno        = labeltable[pcltab[pc].labelno]
macro getlabelno2       = labeltable[pcltab[pc+1].labelno]
macro getstaticaddr     = pcltab[pc].def.staticaddr
macro getstaticaddr2    = pcltab[pc+1].def.staticaddr
macro getstaticaddr3    = pcltab[pc+2].def.staticaddr
macro getframeoffset    = pcltab[pc].def.frameoffset
macro getframeoffset2   = pcltab[pc+1].def.frameoffset
macro getframeoffset3   = pcltab[pc+2].def.frameoffset
macro getprocindex      = pcltab[pc].def.pcindex
macro getdef            = pcltab[pc].def

macro getswmin          = pcltab[pc].swmin
macro getswmax          = pcltab[pc].swmax
macro getnargs          = pcltab[pc].nargs
macro getnvars          = pcltab[pc].nvars
macro getnlocals        = pcltab[pc].nlocals
macro getnparams        = pcltab[pc].nparams
macro getnretvals       = pcltab[pc].nretvalues
macro getstep           = pcltab[pc].step
macro getscale          = pcltab[pc].scale
macro getoffset         = pcltab[pc].poffset
macro getpopone         = pcltab[pc].popone
macro getx              = pcltab[pc].x
macro getparam1         = -2                !frame offset of first param

macro getmemsize        = pcltab[pc].memsize

macro getopcode         = pcltab[pc].jcode
macro steppc            = ++pc
macro steppc2           = pc+:=2
macro steppc3           = pc+:=3
...
dojumpcond:
    ...
    when jdivto_r64 then
        xx:=xstack[sp--]
        pr64:=pstack[sp--]
        pr64^:=pr64^/xx
        steppc

    when jdivto_r32 then
        unimpl
        steppc
