...
// NOTE: MX programs currently are generated to run in low memory below 2GB
// So allocated executable memory must be below 2GB too
// However, when this C version is compiled with gcc, it will run in high memory,
// and memory allocations seems to be in high memory too.
...
void setspecialglobals(int cmdskip) {
    for (int i=1; i<=nsymbols; ++i) {
        ...
        if (strcmp(symbolnametable[i], "msys.$cmdskip")==0) {
            *(byte*)symboladdress[i] = cmdskip;
        }
    }
}
