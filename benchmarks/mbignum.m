...
!CPL =EXPONA, =STBLZ,=BN_ISZERO(DEST)

	dest^.expon:=expona-stblz
IF BN_ISZERO(DEST) THEN DEST.EXPON:=0 FI
...
!   s:=t:=bn_alloc(nchars)
	...
	for i:=0 to upper do
!	   t++^:='*'
		n:=sprintf(t,(i>0 or prel|digitfmt|"%lld"),a^.num^[i])
		t+:=n
...
GLOBAL PROC BNTEST(bignum a)=

CPL "BNTEST",=A.NUMTYPE,=A.LENGTH,=A.EXPON,=A.NEG,=A.NUM
